...
export const fetchAqiData = async (
  ...
  try {
    ...
    if (data && data[0]) {
      // 提取地點名稱
      const locationName = `${data[0].county} ${data[0].sitename}`;
      console.log("Location Name:", locationName); // 確認地點名稱是否正確

      // 提取 AQI 數值和等級，直接取用第一個時間點的資料
      const aqiData = data[0].aqi;
      console.log("AQI Data:", aqiData); // 確認 AQI 數值是否正確

      const aqiLevelData = data[0].status;
      ...
      console.error("Unexpected API response structure:", data); // 確認 API 回應結構
      throw new Error("Unexpected API response structure");
    }
  } catch (error) {
